...
*** Test Cases ***
################### Post ###################
CoapApiDelta_TST_F3_0_2_008_Error_SensorNotFound
	...
	#Send Report
    ${deltaResponse}=    AsgardAPI DeltaAsgardApp    ${url_config}?${ASGARD_COAPAPI_VALUE_TST_F3_1_0_008_SENSOR_INVALID}    ${ASGARD_COAPAP_IMAGE_POPUP_SUCCESS} 
	...
	${body}=    Set Variable    {"ThingToken":"${thingToken}${ASGARD_COAPAPI_VALUE_TST_F3_1_0_003_THINGTOKEN_INVALID}","IpAddress":"${ASGARD_COAPAPP_IP_ADDRESS}","Sensor":"${ASGARD_COAPAPI_VALUE_TST_F3_1_0_008_SENSOR_INVALID}"}
	${endPointName_detail_list}    Create List    ${ENDPOINTNAME_THINGS}	
	AsgardAPI Log Delta    2        ${CODE_40400}    ${RESULTDESC_REQUESTED_OPERATION_COULDNOTBEFOUND_ERROR}    ${CODE_40400}    ${RESULTDESC_REQUESTED_OPERATION_COULDNOTBEFOUND_ERROR}    ${createResponse}    ${thingToken}${ASGARD_COAPAPI_VALUE_TST_F3_1_0_003_THINGTOKEN_INVALID}    ${identity}    ${custom}    ${body}    ${endPointName_detail_list}    ${EMPTY}    ${EMPTY}
