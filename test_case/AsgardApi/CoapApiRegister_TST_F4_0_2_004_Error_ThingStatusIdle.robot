...
*** Test Cases ***
################### Post ###################
CoapApiRegister_TST_F4_0_2_004_Error_ThingStatusIdle  
	...
	...    8.Core : Remove ThingFromAccount
	...
	${custom}=    Set Variable    {"Imei":null,"url":"coapapis.magellan.svc.cluster.local${ASGARD_COAPAPI_URL_REGISTER}","Imsi":["${IMSI}"],"IpAddress":"${ASGARD_COAPAPP_IP_ADDRESS}","ThingID":null}
	${body}=    Set Variable    {"Imsi":"${IMSI}","IpAddress":"${ASGARD_COAPAPP_IP_ADDRESS}"}
	${endPointName_detail_list}    Create List    ${ENDPOINTNAME_THINGS} 
	AsgardAPI Log Register    ${CODE_40400}    ${RESULTDESC_REQUESTED_OPERATION_COULDNOTBEFOUND_ERROR}    ${CODE_40400}    ${RESULTDESC_REQUESTED_OPERATION_COULDNOTBEFOUND_ERROR}    ${createResponse}    ${IMSI}    ${identity}    ${custom}    ${body}    ${endPointName_detail_list}
