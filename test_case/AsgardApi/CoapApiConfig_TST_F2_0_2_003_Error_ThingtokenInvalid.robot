...
*** Test Cases ***
################### Post ###################
CoapApiConfig_TST_F2_0_2_003_Error_ThingtokenInvalid
	...
	${url}=    Replace Parameters Url Path     ${ASGARD_COAPAPP_URL}    ${ASGARD_COAPAPP_URL_CONFIG}    ${ASGARD_COAPAPP_FIELD_TOKEN}    ${thingToken}${ASGARD_COAPAPI_VALUE_TST_F2_1_0_003_THINGTOKEN_INVALID}    ${ASGARD_COAPAPP_FIELD_IPADDRESS}    ${ASGARD_COAPAPP_IP_ADDRESS}
	#Send Config
    AsgardAPI ConfigAsgardApp    ${url}    ${ASGARD_COAPAP_IMAGE_RESPONSE_ERROR_40300}  
	...
	${body}=    Set Variable    {"ThingToken":"${thingToken}${ASGARD_COAPAPI_VALUE_TST_F2_1_0_003_THINGTOKEN_INVALID}","IpAddress":"${ASGARD_COAPAPP_IP_ADDRESS}"}
	${endPointName_detail_list}    Create List    ${ENDPOINTNAME_THINGS} 
	#Send Log
	AsgardAPI Log Config    1     ${CODE_40400}    ${RESULTDESC_REQUESTED_OPERATION_COULDNOTBEFOUND_ERROR}    ${CODE_40400}    ${RESULTDESC_REQUESTED_OPERATION_COULDNOTBEFOUND_ERROR}    ${createResponse}    ${thingToken}${ASGARD_COAPAPI_VALUE_TST_F2_1_0_003_THINGTOKEN_INVALID}    ${identity}    ${custom}    ${body}    ${endPointName_detail_list}    ${EMPTY}
